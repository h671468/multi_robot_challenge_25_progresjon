...
class RightWallFollower:
    ...
    # --- ROBOTENS TILSTANDER  ---
    STATE_TURN_LEFT = 0       # 0: SVING HØYRE (Kollisjonsunngåelse/Innvendig hjørne)
    STATE_FOLLOW_WALL = 1     # 1: FØLG VEGGEN (Normal kjøring)
    STATE_TURN_RIGHT = 2      # 2: SVING VENSTRE (Veggen er borte/Søk)
    
    # --- INNSTILLINGER  ---
    WALL_DISTANCE = 0.5       #  Avstand fra veggen (meter)
    FRONT_THRESHOLD = 0.8     # Avstand som utløser sving HØYRE
    KP_ANGULAR = 0.5          # Styrke på korrigering for å holde avstand
    LINEAR_SPEED = 0.3        # Fart fremover (m/s)
    MAX_RANGE = 3.5           
    TURN_LEFT_SPEED_ANG = 0.8 # Svingehastighet for høyresving (økt)
    TURN_LEFT_SPEED_LIN = 0.2 # Høyere fart fremover under sving
    ...
    def __init__(self, node_ref: Node):
        self.node = node_ref
        self.node.get_logger().info('RightWallFollower-klassen er klar.')
    ...
    def process_scan(self, msg: LaserScan):
        ...
        # --- DELER SENSOR-DATA INN I REGIONER FOR HØYRE VEGGFØLGING ---
        self.regions = {
        ...
        self.node.get_logger().info(
            f"Høyre veggfølger - front={self.regions['front']:.2f}, right={self.regions['right']:.2f}, back_right={self.regions['back_right']:.2f}"
        )
    ...
    def decide_state(self):
        ...
        if self.state == self.STATE_TURN_LEFT and self.turn_start_time is not None:
        ...
        elif d_right > (self.WALL_DISTANCE + 0.3) and d_back_right > (self.WALL_DISTANCE + 0.3):
    ...
    def take_action(self):
        ...
        self.node.get_logger().info(f"Høyre veggfølger - State: {state_str}")

        # SVING HØYRE (Innvendig hjørne)
        if self.state == self.STATE_TURN_LEFT:
            linear_x = 0.0  # Stopp fremover under sving
            angular_z = -self.TURN_LEFT_SPEED_ANG 
        ...
        # FØLG VEGGEN (P-kontroll)
            ...
            # P-kontroll (sving høyre når for nær veggen, venstre når for langt fra)
        ...
        self.node.get_logger().info(
            f"Høyre veggfølger - Cmd_vel: linear.x={linear_x:.2f}, angular.z={angular_z:.2f}"
        )
